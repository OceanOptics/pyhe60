...
def _make_lut_chunk_multiprocessing(cfg: tuple, constants: dict, varnames: Union[str, List[str]],
    ...
    :param varnames: Variables name to store in LUT (e.g. KLu, Rrs, Lu, Ed, Kd)
    ...
    # Run HydroLight 6.0
    dfs = run_he60_oneshot({**input, **constants}, varnames, working_dir, prefix=run_hash, flag_cleanup=True)

    return dfs, index


def make_lut_multiprocessing(varnames: Union[str, List[str]], dimensions: dict, constants: dict = None,
                             path_to_lut: str = None, working_dir: str = HE60_DATA,
                             resume_run: bool=False, lut_is_contiguous: bool=False, generate_inputs_fn=generate_inputs,
    ...
    :param generate_inputs_fn: Function for generating input variables (default: generate_inputs)
    :param n_processes: Number of processes to use (default: None -> os.process_cpu_count() )
    :return:
    """
    ...
    # Generate input
    inputs, indexes = generate_inputs_fn(dimensions, constants)
    if resume_run:
        inputs, indexes = skip_computed_chunks(inputs, indexes, varnames[0], path_to_lut, lut_is_contiguous)
        print('Resuming run, remaining simulations:', len(inputs))

    # Initialize LUT
    if not resume_run:
        init_lut(path_to_lut, varnames, dimensions, constants, lut_hash)
    ...
    with Pool(processes=n_processes) as pool:  # default to os.process_cpu_count()
        for chunk in tqdm(pool.imap_unordered(worker, zip(inputs, indexes)), total=len(inputs),
                          desc=f'HE60', smoothing=0):
            dfs, index = chunk
            if dfs:
                write_chunk_to_lut(dfs, index, varnames, path_to_lut)
