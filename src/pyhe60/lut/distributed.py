...
@ray.remote
def _make_lut_chunk_ray(index: Tuple[int], input: dict, constants: dict, varnames: str,
    ...
    :param varnames: Variables names to store in LUT (e.g. KLu, Rrs, Lu, Ed, Kd)
    ...
    dfs = run_he60_oneshot({**input, **constants}, varnames, node_working_dir, prefix=run_hash, flag_cleanup=True)
    return dfs, index


def make_lut_ray(varnames: str, dimensions: dict, constants: dict = None,
    ...
    pbar = tqdm(total=len(inputs), desc="HE60", unit="runs", smoothing=0) if tqdm else None
    try:
        pending = []
        constants_ref = ray.put(constants or {})
        for index, input in zip(indexes, inputs):
            pending.append(_make_lut_chunk_ray.remote(index, input, constants_ref, varnames, lut_hash, node_working_dir))
            if max_in_flight and len(pending) >= max_in_flight:
                done, pending = ray.wait(pending, num_returns=1)
                dfs, index = ray.get(done[0])
                if dfs:
